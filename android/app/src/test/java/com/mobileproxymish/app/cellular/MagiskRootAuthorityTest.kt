...
class MagiskRootAuthorityTest {
    ...
    fun readyRequiresRootAndRpdbRead() {
        val authority = MagiskRootAuthority.forTesting(
            FakeProcess(
                ...
                RootProcessResult(0, "10000: from all lookup main\n"),
            ),
        )
    ...
    @Test
    fun missingSuIsUnavailable() {
        val authority = MagiskRootAuthority.forTesting(
            FakeProcess(RootProcessResult(127, "not found")),
        )

        assertEquals(RootAuthorityStatus.Unavailable, authority.probe())
    }
