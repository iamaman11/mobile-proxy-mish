    ...
    try {
        while (-not $process.HasExited) {
            ...
            if ($null -ne $observation) {
                ...
                elseif ([int]$observation.pid -ne [int]$observationPid) {
                    if ($stimulusStarted -eq 0) {
                        $observations.Clear()
                        $observationPid = [int]$observation.pid
                    }
                    else {
                        Stop-MishRecovery 'LAB_DNS_LIFETIME_PROCESS_CHANGED' 'Canonical DNS observation changed PRODUCT PID after the live proxy-domain stimulus started.'
                    }
                }
                ...
                if (
                    -not $credentialAttempted -and
                    $watch.ElapsedMilliseconds -ge 1000 -and
        ...
        return [pscustomobject]@{
            ...
            DnsLifetime = [ordered]@{
                ...
                started_delta = if ($null -ne $first -and $null -ne $last) { [int64]$last.started - [int64]$first.started } else { 0 }
