...
const CANONICAL_LISTENERS: [ProxyListener; 3] = [
    ProxyListener { protocol: ProxyProtocol::Mixed, port: MIXED_PORT },
    ProxyListener { protocol: ProxyProtocol::Socks5, port: SOCKS5_PORT },
    ProxyListener { protocol: ProxyProtocol::Http, port: HTTP_CONNECT_PORT },
];
...
impl ProxyCredentialMaterial {
    pub fn new(username: impl Into<String>, password: impl Into<String>) -> Result<Self, ProxyPolicyError> {
        let username = username.into();
        let password = password.into();
        if username.trim().is_empty() { return Err(ProxyPolicyError::EmptyUsername); }
        if password.trim().is_empty() { return Err(ProxyPolicyError::EmptyPassword); }
...
impl fmt::Debug for ProxyCredentialMaterial {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.debug_struct("ProxyCredentialMaterial")
...
impl ProxyServingPlan {
    pub fn canonical(listen_address: IpAddr, credentials: ProxyCredentialMaterial) -> Result<Self, ProxyPolicyError> {
        if listen_address.is_unspecified() { return Err(ProxyPolicyError::WildcardListenAddress); }
        Ok(Self { listen_address, credentials })
    }
    pub fn listen_address(&self) -> IpAddr { self.listen_address }
    pub fn listeners(&self) -> &'static [ProxyListener; 3] { canonical_listeners() }
    pub fn credentials(&self) -> &ProxyCredentialMaterial { &self.credentials }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProxyPolicyError { WildcardListenAddress, EmptyUsername, EmptyPassword }
...
mod tests {
    use super::*;
    fn credentials() -> ProxyCredentialMaterial { ProxyCredentialMaterial::new("ci-user", "ci-password").expect("valid credentials") }
    #[test]
    fn canonical_listener_contract_is_fixed() {
        assert_eq!(canonical_listeners(), &[
            ProxyListener { protocol: ProxyProtocol::Mixed, port: 1080 },
            ProxyListener { protocol: ProxyProtocol::Socks5, port: 1081 },
            ProxyListener { protocol: ProxyProtocol::Http, port: 3128 },
        ]);
        let plan = ProxyServingPlan::canonical(IpAddr::from([127,0,0,1]), credentials()).expect("explicit address");
        assert_eq!(plan.listeners(), canonical_listeners());
    }
    #[test]
    fn wildcard_ipv4_and_ipv6_are_rejected() {
        for address in ["0.0.0.0", "::"] {
            let error = ProxyServingPlan::canonical(address.parse().expect("valid IP"), credentials()).expect_err("wildcard must fail closed");
            assert_eq!(error, ProxyPolicyError::WildcardListenAddress);
        }
    }
    #[test]
    fn explicit_loopback_is_allowed_for_bounded_local_acceptance() {
        let plan = ProxyServingPlan::canonical(IpAddr::from([127,0,0,1]), credentials()).expect("explicit loopback");
        assert_eq!(plan.listen_address(), IpAddr::from([127,0,0,1]));
    }
    #[test]
    fn public_authentication_material_cannot_be_empty() {
        assert_eq!(ProxyCredentialMaterial::new("", "password"), Err(ProxyPolicyError::EmptyUsername));
        assert_eq!(ProxyCredentialMaterial::new("user", "  "), Err(ProxyPolicyError::EmptyPassword));
    }
    #[test]
    fn credential_debug_output_is_redacted() {
        let material = ProxyCredentialMaterial::new("visible-user", "secret-password").expect("valid credentials");
