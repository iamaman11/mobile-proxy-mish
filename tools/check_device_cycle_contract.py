...
def main() -> None:
    ...
    for token in (
        r"C:\mish-lab\tools\powershell-7.6.6\pwsh.exe",
        "-NoLogo -NoProfile -NonInteractive -Command",
        "{0}",
    ):
        if token not in physical_text:
            raise SystemExit(
                "device cycle contract: DEVICE-1 jobs must execute through pinned portable "
                f"PowerShell 7.6.6 command shell; missing {token!r}"
            )
