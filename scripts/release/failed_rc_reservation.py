...
def verify(*, tag: str, tag_sha: str, runs_path: Path, jobs_path: Path) -> dict[str, object]:
    ...
    matching = [
        ...
        and run["actor"].get("login") == "github-actions[bot]"
    ]
    if len(matching) != 1:
        raise ValueError("previous unpublished RC must have exactly one failed machine-owned workflow_dispatch run")
    ...
    publish_jobs = [
        ...
        and job.get("name") == "Publish immutable Android RC"
